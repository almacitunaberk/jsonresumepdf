...
module.exports = (
  ...
  if (formatToUse === '.html') {
    createHtml(resumeJson, fileName, theme, formatToUse, (error, buff) => {
      if (error) {
        console.error(error, '`createHtml` errored out');
      }
      callback(error, buff, fileName, formatToUse);
    });
  } else if (formatToUse === '.pdf') {
    createPdf(resumeJson, fileName, theme, formatToUse, (error, buff) => {
      if (error) {
